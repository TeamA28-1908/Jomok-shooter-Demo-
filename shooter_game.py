...
load_time = False
num_fire = 0
#the flag is reset by the window close button
while run:
   ...
   for e in event.get():
       ...
       #event of pressing the spacebar - the sprite shoots
           ...
           if e.key == K_SPACE:
               if num_fire < 5 and load_time == False:
                   num_fire = num_fire + 1
                   fire_sound.play()
                   ship.fire()

               if num_fire >= 200 and load_time == False:
                   last_time = timer()
                   load_time = True
 #the game itself: actions of sprites, checking the rules of the game, redrawing
   if not finish:
       ...
       if load_time == True:
           ...
           if now_time - last_time < 3:
               reload = font2.render("WOI RELOAD", 1, (150, 0, 0))
   ...
   #bonus: automatic restart of the game
       ...
       for i in range(1, 6):
           ...
           monsters.add(monster)
       for i in range(1, 3):
           asteroid = Enemy("asteroid.png", randint(30,) -40, 80, 50, randint(1, 7))
           asteroids.add(asteroid)
